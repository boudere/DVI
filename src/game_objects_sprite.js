...
class GameObjectsSprite extends Phaser.GameObjects.Sprite {
    ...
    async start_animation(animation) {
        ...
        this._active_tweens = []; // almacena tweens activos y sus datos
        this._skip_requested = false;
        ...
        for (const [key, value] of Object.entries(animation_data)) {
            ...
            await Promise.all(animation_tweens); // espera al grupo
        }
    ...
    run_tween(animation_data) {
        ...
        if ('scaleX_end' in animation_data) tweenProps.scaleX = animation_data.scaleX_end;
        if ('scaleY_end' in animation_data) tweenProps.scaleY = animation_data.scaleY_end;
        ...
        return new Promise((resolve) => {
            ...
            // guardar el tween y datos finales por si hay skip
            this._active_tweens.push({ tween, finalValues: tweenProps, resolve });
        });
    }
